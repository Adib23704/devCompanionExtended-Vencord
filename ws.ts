...
async function buildPatchIndex(): Promise<void> {
  ...
  patchIndex.inFlight = (async () => {
    ...
    for (let i = 0; i < moduleIds.length; i += batchSize) {
      const batch = moduleIds.slice(
        i,
        Math.min(i + batchSize, moduleIds.length),
      );
    ...
    if (queries.length) {
      ...
      for (const query of queries) {
        try {
          ...
          debugInfo(
            `Search prewarm failed for "${query}" (${source}): ${String(error)}`,
          );
        }
      }
    }
...
const storeWatchers = new Map<
  string,
  { store: any; handler: () => void; remove: () => void }
>();
...
type AnalyticsTarget = {
  trackWithMetadata?: (...args: any[]) => unknown;
...
async function searchModulesInternal(args: {
  kind: string;
  pattern: string;
  limit?: number;
  isRegex?: boolean;
...
async function searchExtractInternal(args: {
  ...
  if (kind === "literal") {
    ...
    const result = await handleSearchByPatternAsync({
      pattern,
      limit: searchLimit,
    });
...
function shouldStreamBatch(args: Record<string, unknown>): boolean {
  ...
  const requests = Array.isArray(args.requests)
    ? (args.requests as BatchToolRequest[])
    : [];
  return requests.some(
    (req) => typeof req.tool === "string" && expectedSlowTools.has(req.tool),
  );
}
...
function batchToolsStream(args: Record<string, unknown>): ToolCallResult {
  ...
  const parallelism = Math.max(
    1,
    Math.min((args.parallelism as number | undefined) ?? 4, 10),
  );
  const defaultTimeoutMs = Math.max(
    1000,
    Math.min((args.timeoutMs as number | undefined) ?? 60000, 120000),
  );
  ...
  const resourceId = storeResource(
  ...
  const runNext = async () => {
    ...
    if (tool === "batch_tools") {
      results.push({
        ...
        tool,
        ok: false,
        error: "batch_tools cannot call itself",
      });
      summary.failed += 1;
      summary.completed += 1;
      update();
    ...
    const timeoutMs = Math.max(
      500,
      Math.min(request.timeoutMs ?? defaultTimeoutMs, 120000),
    );
    try {
  ...
  const runners = Array.from(
    { length: Math.min(parallelism, pending.length) },
    runNext,
  );
...
async function computeToolResult(
  name: string,
  ...
  switch (name) {
    case "module": {
      ...
      if (action === "size") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for size");
      ...
      if (action === "extract") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for extract");
      ...
      if (action === "exports") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for exports");
        return handleGetModuleExports({ moduleId });
      }
      if (action === "explain") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for explain");
        const usePatched = toOptionalBoolean(args.usePatched) ?? true;
      ...
      if (action === "findStrings") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for findStrings");
        const usePatched = toOptionalBoolean(args.usePatched) ?? true;
        const source = extractModule(moduleId, usePatched);
      ...
      if (action === "context") {
        if (args.pattern && typeof moduleId !== "number") {
          const searchLimit =
            toOptionalNumber(args.searchLimit) ?? toOptionalNumber(args.limit);
        ...
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for context");
        if (args.pattern) {
          const usePatched = toOptionalBoolean(args.usePatched) ?? true;
        ...
        return getModuleContextInternal(
          moduleId,
          toOptionalBoolean(args.usePatched) ?? true,
        );
      }
      if (action === "diff") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for diff");
        return handleDiff({ moduleId });
      }
      if (action === "deps" || action === "relationships") {
        if (typeof moduleId !== "number")
          throw new Error("moduleId is required for relationships");
        return handleGetModuleRelationships({ moduleId });
      }
      if (Array.isArray(args.props) && args.props.length) {
        if (args.all)
          return handleFindAll({ props: args.props, limit: args.limit });
      ...
      if (args.pattern) {
        return handleSearchByPatternAsync({
          pattern: String(args.pattern),
          limit: toOptionalNumber(args.limit),
        });
      }
    ...
    case "store": {
      const action = String(
        args.action ?? (args.name || args.storeName ? "state" : "list"),
      );
      ...
      if (action === "subscriptions") {
        return handleGetStoreSubscriptions({
          storeName,
          limit: toOptionalNumber(args.limit),
        });
      }
    ...
    case "intl": {
      ...
      if (action === "search") {
        ...
        return handleGetIntlKeys({
          filter: String(args.query),
          limit: toOptionalNumber(args.limit),
        });
      }
      if (action === "scan") {
    ...
    case "flux": {
      ...
      if (action === "events")
        return handleGetFluxEvents({ filter: args.filter });
      if (action === "types")
        return handleDispatcherActions({
          filter: args.filter,
          isRegex: args.isRegex,
        });
      if (action === "dispatch") {
        if (!args.type) throw new Error("type is required for dispatch");
        return handleDispatchFlux({
          action: { type: String(args.type), ...(args.payload ?? {}) },
        });
      }
      if (action === "listeners") {
        return handleDispatcherActions({
          filter: args.event ?? args.filter,
          isRegex: args.isRegex,
        });
      }
    ...
    case "patch": {
      ...
      if (action === "test") {
        ...
        const replacements =
          Array.isArray(args.replacements) && args.replacements.length
            ? args.replacements
            : args.match
              ? [
                  {
        ...
        if (!replacements.length)
          throw new Error("replacements or match is required");
      ...
      if (action === "unique") {
        ...
        const uniques = (result.matches ?? []).filter(
          (m: { occurrences?: number }) => m.occurrences === 1,
        );
      ...
      if (action === "plugin") {
        const resolved = resolvePluginName(
          toOptionalString(args.pluginName) ??
            toOptionalString(args.pluginId) ??
      ...
      if (action === "suggest") {
        ...
        return handleFindPatchTargets({
          pattern: String(pattern),
          maxResults: toOptionalNumber(args.limit),
        });
      }
    ...
    case "discord": {
      ...
      if (action === "snowflake")
        return handleDecodeSnowflake({ snowflake: args.snowflake });
    ...
    case "component": {
      ...
      if (action === "find") {
        if (Array.isArray(args.code) && args.code.length) {
          return handleSearch({
            findType: "findComponentByCode",
            args: args.code.map(parseRegexString),
          });
        }
    ...
    case "search": {
      ...
      if (action === "extract") {
        if (!args.kind || !args.pattern)
          throw new Error("kind and pattern are required");
      ...
      if (
        ["props", "code", "store", "component", "moduleId"].includes(action)
      ) {
        return searchModulesInternal({
          kind: action,
          pattern: String(args.pattern),
    ...
    case "batch_tools": {
      ...
      for (const request of requests) {
        ...
        if (tool === "batch_tools") {
          results.push({
            id: request.id as string | undefined,
            tool,
            ok: false,
            error: "batch_tools cannot call itself",
          });
...
const MCP_TOOL_HANDLERS: Record<string, MessageHandler> = {
  ...
  component: (data) =>
    callMcpTool("component", data as Record<string, unknown>),
  search: (data) => callMcpTool("search", data as Record<string, unknown>),
  trace: (data) => callMcpTool("trace", data as Record<string, unknown>),
  intercept: (data) =>
    callMcpTool("intercept", data as Record<string, unknown>),
  analytics: (data) =>
    callMcpTool("analytics", data as Record<string, unknown>),
  evaluateCode: (data) =>
    callMcpTool("evaluateCode", data as Record<string, unknown>),
  read_resource: (data) =>
    callMcpTool("read_resource", data as Record<string, unknown>),
  reloadDiscord: (data) =>
    callMcpTool("reloadDiscord", data as Record<string, unknown>),
  waitForIpc: (data) =>
    callMcpTool("waitForIpc", data as Record<string, unknown>),
  testPatch: (data) =>
    callMcpTool("testPatch", data as Record<string, unknown>),
  batch_tools: (data) =>
    callMcpTool("batch_tools", data as Record<string, unknown>),
};
  ...
  switch (request.method) {
    ...
    case "tools/list":
      return {
        jsonrpc: "2.0",
        id: request.id ?? null,
        result: { tools: getMcpTools(Object.keys(MCP_TOOL_HANDLERS)) },
      };
    case "tools/call": {
...
async function findModuleIdAsync(
  code: string,
): Promise<number | { multiple: number[]; count: number }> {
...
function togglePlugin(pluginName: string): void {
  ...
  if (!result) {
    ...
    throw new Error(
      `Error while ${wasEnabled ? "stopping" : "starting"} plugin`,
    );
  }
...
async function handleSearch(requestData: unknown) {
  ...
  switch (findType) {
    ...
    case "findByCode":
      results = findAll(
        filters.byCode(...(parsedArgs as Array<string | RegExp>)),
      );
      break;
    case "findStore":
    ...
    case "findComponentByCode":
      results = findAll(
        filters.componentByCode(...(parsedArgs as Array<string | RegExp>)),
      );
      break;
    case "findModuleId":
      results = Object.keys(search(parsedArgs[0] as string | RegExp)).map(
        (id) => wreq.m[id],
      );
      break;
    default:
  ...
  if (uniqueResults.length > 1) {
    logger.warn(
      `Found ${uniqueResults.length} results for ${findType} (${time}ms)`,
    );
...
function handleTestPatch(requestData: unknown) {
  const {
    ...
    contextLines = 2,
    radius = 120,
  } = requestData as TestPatchRequest;
  if (!find) {
  ...
  const clampContextLines = Math.max(
    0,
    Math.min(10, Number(contextLines ?? 2)),
  );
  ...
  for (let i = 0; i < replacements.length; i++) {
    ...
    if (preview) {
      const rawContext = getMatchContext(
        rawSrc,
        matchPattern,
        clampContextLines,
        clampRadius,
      ...
      if (
        (previewMode === "full" || previewMode === "context-only") &&
        flatContext.found
      ) {
        if (previewMode === "context-only") {
          ...
          const anchorIndex =
            findLookbehindAnchor(flatOriginal, matchPattern) ??
            flatContext.index ??
            0;
  ...
  try {
    const findContext = getMatchContext(
      rawSrc,
      findPattern,
      clampContextLines,
      clampRadius,
    ...
    if (findContext.found) {
      const snippet = [
        findContext.context?.before?.join(" "),
        findContext.context?.current,
        findContext.context?.after?.join(" "),
      ]
        .filter(Boolean)
        .join(" ");
...
function handlePatchLint(requestData: unknown) {
  ...
  const scores = [
    findAnalysis.score,
    ...replacementAnalyses.map((r) => r.score),
  ];
  const overallScore = Math.max(
    1,
    Math.min(10, Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)),
  );
  const warnings = [
  ...
  if (!suggestions || suggestions.anchors.length === 0) {
    ...
    if (fallbackAnchors.length)
      suggestions = { anchors: fallbackAnchors.slice(0, 6) };
  }
...
function handleTogglePlugin(requestData: unknown) {
  ...
  const { pluginName, enabled, pluginId, name } =
    requestData as PluginToggleRequest & { pluginId?: string; name?: string };
...
async function handleGetStores(requestData?: unknown) {
  ...
  if (
    storeCache.data &&
    Date.now() - storeCache.timestamp < STORE_CACHE_TTL_MS
  ) {
  ...
  const storeInfo: Record<
    string,
    { found: boolean; moduleId?: number; source?: string }
  > = {};
  const registerStore = (
    name: string | null,
    moduleId?: number,
  ...
  for (const [name, store] of fluxStores.entries()) {
    ...
    registerStore(
      name,
      typeof moduleId === "number" ? moduleId : undefined,
      "fluxStores",
    );
  }
  ...
  for (const value of Object.values(Common)) {
    maybeRegisterStore(
      value,
      (value as any)?.__moduleId ?? (value as any)?._moduleId,
      "common",
    );
  }
      ...
      requestAnimationFrame(async () => {
        for (const query of chunk) {
          try {
            ...
            switch (findType) {
              case "findByProps":
                moduleResults = findAll(
                  filters.byProps(...(parsedArgs as string[])),
                );
                break;
              case "findByCode":
                moduleResults = findAll(
              ...
              case "findStore":
                moduleResults = findAll(
                  filters.byStoreName(parsedArgs[0] as string),
                );
                break;
              case "findComponentByCode":
                moduleResults = findAll(
                  filters.componentByCode(
                    ...(parsedArgs as Array<string | RegExp>),
                  ),
                );
                break;
              case "findModuleId":
                moduleResults = Object.keys(
                  search(parsedArgs[0] as string | RegExp),
                ).map((id) => wreq.m[id]);
                break;
              default:
            ...
            if (unique.length === 1) {
              const foundCode = (
                unique[0] as { toString(): string }
              ).toString();
              const moduleId = findModuleIdFast(foundCode);
              results.push({
                query: query.name || query.findType,
                moduleId,
                found: true,
              });
              found++;
              ...
              results.push({
                ...
                moduleId: null,
                found: false,
                error: `Found ${unique.length} results`,
              });
              failed++;
            }
          } catch (err) {
...
async function handleGetPatchedModulesAsync(requestData: unknown) {
  const {
    ...
    offset = 0,
    limit = 100,
  } = requestData as { pluginName?: string; offset?: number; limit?: number };
  ...
  const task = (async () => {
    ...
    if (useIndex && patchIndex.ready) {
      for (const entry of patchIndex.entries) {
        if (normalizedName && !entry.patchedBy.includes(normalizedName))
          continue;
        totalFound++;
...
async function handleSearchByPatternAsync(requestData: unknown) {
  const { pattern, limit = 20 } = requestData as {
    pattern: string;
    limit?: number;
  };
  const startTime = performance.now();
  ...
  const candidateIds =
    !isRegex && settings.store.prebuildSearchIndex
  ...
  const allModuleIds = candidateIds
  ...
  for (let i = 0; i < allModuleIds.length; i += batchSize) {
    ...
    for (const id of batch) {
      ...
      const matches = isRegex
        ? (searchPattern as RegExp).test(code)
        : code.includes(searchPattern as string);
      if (matches) {
        modules.push({
          id: Number(id),
          preview: code.substring(0, 200),
...
function handleGetPluginList(requestData: unknown) {
  const { filter, showPatches } = (requestData ?? {}) as {
    filter?: string;
    showPatches?: boolean;
  };
  const plugins: Array<{
  ...
  for (const [name, plugin] of Object.entries(Vencord.Plugins.plugins)) {
    ...
    const patches =
      (plugin as { patches?: Array<{ find: unknown; replacement?: unknown }> })
        .patches ?? [];
    const entry: {
      name: string;
      enabled: boolean;
    ...
    if (showPatches) {
      entry.patchDetails = patches.slice(0, 10).map((patch) => ({
        find: String(
          typeof patch.find === "string" ? patch.find : patch.find,
        ).slice(0, 120),
        replacements: Array.isArray(patch.replacement)
...
function handleGetPluginInfo(requestData: unknown) {
  const { pluginName, pluginId, name } = requestData as {
    pluginName?: string;
    pluginId?: string;
    name?: string;
  };
  ...
  return {
    ...
    dependencies: safeSerialize(
      (plugin as { dependencies?: unknown }).dependencies ?? [],
    ),
...
function handleGetModuleRelationships(requestData: unknown) {
  ...
  const exports = (code.match(/exports?\.([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [])
    .slice(0, 30)
    .map((m) => m.match(/exports?\.([a-zA-Z_$][a-zA-Z0-9_$]*)/)?.[1] || "");
...
function handleModuleContext(requestData: unknown) {
  ...
  if (regex) {
    ...
    const matchDetails = matches.slice(0, maxSnippets).map((m) => {
      ...
      return {
        ...
        snippet: code
          .slice(
            Math.max(0, index - clampRadius),
            Math.min(code.length, index + (m[0]?.length || 0) + clampRadius),
          )
          .replace(/\s+/g, " "),
  ...
  while (idx !== -1 && offsets.length < maxSnippets) {
    ...
    matchDetails.push({
      ...
      context:
        clampContextLines > 0
          ? getLineContext(code, idx, clampContextLines)
          : undefined,
    });
...
async function handleSearchContext(requestData: SearchContextRequest) {
  ...
  for await (const { id, code } of ModuleSearchEngine.searchPattern(
    searchPattern as string | RegExp,
  )) {
    const offsets: number[] = [];
    let matchCount = 0;
    if (regex) {
    ...
    const matches = offsets.map((offset) => {
      ...
      const snippet = code
        .slice(
          Math.max(0, offset - clampRadius),
          Math.min(
            code.length,
            offset + (patternString?.length ?? 0) + clampRadius,
          ),
        )
        .replace(/\s+/g, " ");
      return {
        ...
        context:
          clampContextLines > 0
            ? getLineContext(code, offset, clampContextLines)
            : undefined,
      };
    });
...
async function handleFindReactComponentsAsync(requestData: unknown) {
  const {
    ...
    offset = 0,
    limit = 50,
  } = requestData as { pattern: string; offset?: number; limit?: number };
  ...
  const componentNameRegex =
    /(?:function|const|class|let|var)\s+([A-Z][a-zA-Z0-9_]*)/g;

  for await (const { id, code } of ModuleSearchEngine.searchPattern(pattern)) {
    const hasReact = reactIndicators.some((indicator) =>
...
function handleGetPluginPatches(requestData: unknown) {
  ...
  const pluginPatches =
    (
      plugin as {
        patches?: Array<{ find: string | RegExp; replacement?: unknown }>;
      }
    ).patches || [];
  ...
  for (const patch of pluginPatches) {
    ...
    const replacements = patch.replacement
      ? ((Array.isArray(patch.replacement)
          ? patch.replacement
          : [patch.replacement]) as Array<{ match: string; replace: string }>)
      : [];

    for (const replacement of replacements) {
      if (typeof replacement.replace !== "string") continue;
      const matches = replacement.replace.matchAll(
        /\$self\.([A-Za-z_$][\w$]*)/g,
      );
  ...
  const missingSelfRefs = Array.from(selfRefs).filter(
    (ref) => !(ref in (plugin as unknown as Record<string, unknown>)),
  );
  const warnings = missingSelfRefs.length
    ? missingSelfRefs.map(
        (ref) =>
          `Patch references $self.${ref} but the plugin does not export it.`,
      )
    : [];
...
async function handleModuleStats() {
  ...
  return {
    ...
    largestModule,
    smallestModule:
      smallestModule.size === Infinity ? { id: 0, size: 0 } : smallestModule,
...
async function handleFindUnusedModulesAsync() {
  ...
  const unused: number[] = [];
  for (const moduleId of allModules.slice(
    0,
    Math.min(1000, allModules.length),
  )) {
...
function logToolResult(options: {
  ...
  const resultText = options.error
    ? String(options.error)
    : JSON.stringify(options.responseData ?? {});
  const notFound =
    /"found"\s*:\s*false|"matchCount"\s*:\s*0|No module found|No match\b|No results\b/i.test(
      resultText,
    );
  ...
  const reqArgs =
    options.requestData && typeof options.requestData === "object"
      ? (options.requestData as Record<string, unknown>)
      : {};
...
function handleCreateCustomPatch(requestData: unknown) {
  ...
  try {
    ...
    return {
      ...
      valid: true,
      willMatch: moduleId
        ? wreq.m[moduleId].toString().includes(find)
        : "unknown",
    };
  } catch (err) {
...
function handleTestCustomPatch(requestData: unknown) {
  ...
  try {
    ...
    const diff = {
      before: original.substring(
        Math.max(0, original.indexOf(find) - 100),
        original.indexOf(find) + find.length + 100,
      ),
      after: patched.substring(
        Math.max(0, patched.indexOf(replace) - 100),
        patched.indexOf(replace) + replace.length + 100,
      ),
    };
...
function handleFindPatchTargets(requestData: unknown) {
  ...
  for (const id in wreq.m) {
    ...
    if (matches > 0) {
      ...
      targets.push({
        moduleId: Number(id),
        preview: code.substring(
          Math.max(0, matchIndex - 50),
          Math.min(matchIndex + 150, code.length),
        ),
        matchCount: matches,
        size: code.length,
      });
...
function handleAnalyzePatchOverlap(requestData: unknown) {
  ...
  try {
    ...
    for (const plugin of patchedBy) {
      const pluginPatches =
        (Vencord.Plugins.plugins[plugin] as any)?.patches || [];
      let changeCount = 0;

      for (const patch of pluginPatches) {
        const pattern =
          typeof patch.find === "string" ? patch.find : patch.find.source;
        if (
          original.includes(pattern) ||
          (patch.find instanceof RegExp && patch.find.test(original))
        ) {
          changeCount++;
        }
      }
...
function handlePatchOverlapByFind(requestData: unknown) {
  const { moduleId, find } = requestData as {
    moduleId?: number;
...
function handleAnalytics(requestData: unknown) {
  ...
  if (action === "get") {
    const maxPayloadChars = Number(
      (requestData as { maxPayloadChars?: number }).maxPayloadChars,
    );
    const slice = analyticsState.events
  ...
  if (!analyticsState.active) {
    ...
    const now = Date.now();
    const cached =
      analyticsState.cachedTargets &&
      now - analyticsState.cachedTargets.createdAt <
        ANALYTICS_TARGET_CACHE_TTL_MS
        ? analyticsState.cachedTargets.targets
        : null;
    const targets: Array<{
      target: AnalyticsTarget;
      original: AnalyticsTarget;
    }> = [];
    const collectTargets = (candidates: AnalyticsTarget[]) => {
      ...
      for (const candidate of candidates) {
        ...
        seenTargets.add(obj);
        if (
          typeof candidate.trackWithMetadata === "function" ||
      ...
      try {
        ...
        if (!matcher || matcher(name)) {
          ...
          if (fields && fields.length > 0) {
            entry = ensureBaseFields(
              filterFields(entry, fields) as Record<string, unknown>,
              {
                event: name,
          ...
          const finalEntry = redact
            ? (redactSensitive(entry) as AnalyticsEntry)
          ...
          if (
            analyticsState.stopOnMatch ||
            (typeof analyticsState.stopAfter === "number" &&
              analyticsState.matchCount >= analyticsState.stopAfter)
          ) {
            const reason = analyticsState.stopOnMatch
              ? "stopOnMatch"
              : "stopAfter";
            for (const entry of analyticsState.targets) {
              if (entry.original.trackWithMetadata) {
                entry.target.trackWithMetadata =
                  entry.original.trackWithMetadata;
              }
              if (entry.original.track) {
                entry.target.track = entry.original.track;
              }
    ...
    for (const entry of targets) {
      if (typeof entry.target.trackWithMetadata === "function") {
        entry.target.trackWithMetadata = function (
          event: unknown,
          payload: unknown,
          flush?: unknown,
        ) {
          ...
          return (
            entry.original.trackWithMetadata as (...args: any[]) => unknown
      ...
      if (typeof entry.target.track === "function") {
        entry.target.track = function (event: unknown, payload?: unknown) {
          ...
          return (entry.original.track as (...args: any[]) => unknown).apply(
            this,
            arguments as any,
          );
        };
    ...
    analyticsState.stopAfter =
      typeof stopAfter === "number" && Number.isFinite(stopAfter)
        ? Math.max(1, Math.floor(stopAfter))
        : null;
...
async function handleSearchLiteral(requestData: unknown) {
  const {
    ...
    offset = 0,
    preset = "full",
  } = requestData as SearchLiteralRequest & { offset?: number };
  const results: Array<{
    id: number;
    preview: string;
    occurrences: number;
  ...
  const candidateIds =
    !matcher && searchStr && settings.store.prebuildSearchIndex
      ? await ModuleSearchEngine.getIndexedCandidates(searchStr)
      : null;
  const moduleIds = candidateIds
    ? Array.from(candidateIds)
    : Object.keys(wreq.m);
        ...
        while (idx !== -1 && offsets.length < 3) {
          ...
          snippets.push(
            code
              .substring(
                Math.max(0, idx - 60),
                Math.min(code.length, idx + searchStr.length + 60),
              )
              .replace(/\s+/g, " "),
          );
...
function resolveComponentNameFromModule(
  moduleId: number,
  fallbackName: string | null,
) {
  if (!moduleId) return null;
  if (componentNameCache.has(moduleId))
  ...
  const isGoodName = (name: string | null) =>
    Boolean(name && name.length > 2 && name !== fallbackName);
...
function handleGetStoreMethods(requestData: unknown) {
  ...
  const moduleId =
    (store as any).__moduleId ?? (store as any)._moduleId ?? null;
  return {
    storeName,
    moduleId,
...
function getModuleKeySample(mod: Record<string, unknown>, maxKeys = 20) {
  ...
  return {
    ...
    sample: Object.fromEntries(
      keys.slice(0, maxKeys).map((k) => [k, typeof mod[k]]),
    ),
  };
}
...
async function handleFindByProps(requestData: unknown) {
  ...
  const { keys, sample } = getModuleKeySample(
...
async function handleFindComponentByCode(requestData: unknown) {
  ...
  return {
    ...
    sourcePreview:
      typeof comp === "function" ? comp.toString().slice(0, 5000) : null,
  };
}
...
function handleGetFluxEvents(requestData: unknown) {
  ...
  const handlers =
    dispatcher?._actionHandlers?._orderedActionHandlers ??
    dispatcher?._actionHandlers ??
...
function handleComponentLocator(requestData: unknown) {
  const {
    ...
    isRegex = false,
    limit = 20,
  } = requestData as ComponentLocatorRequest;
  const patternInfo = parsePotentialRegex(query, isRegex);
  const results: Array<{
    name: string;
    moduleId: number;
  ...
  const componentNameRegex =
    /(?:function|class|const|let|var)\s+([A-Z][A-Za-z0-9_]*)/;
    ...
    try {
      ...
      if (matched) {
        ...
        const exports = cache[id]?.exports
          ? Object.keys(cache[id].exports).slice(0, 5)
          : [];
        let patched = false;
        try {
...
function handleDispatcherActions(requestData: unknown) {
  ...
  for (const entry of entries) {
    ...
    if (!regex && patternString && !actionType.includes(patternString))
      continue;
...
function handleEventListenerAudit(requestData: unknown) {
  ...
  const searchPattern = event
    ? new RegExp(
        `addEventListener\\s*\\(\\s*['"]${event.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}['"]`,
        "gi",
      )
...
async function handleEvaluateCode(requestData: unknown) {
  ...
  } = requestData as EvaluateCodeRequest;
  if (!code?.trim()) {
    throw new Error("code is required");
  }
  (
    evalSession.vars as Record<string, unknown> & { reset?: () => boolean }
  ).reset = () => {
    for (const key of Object.keys(evalSession.vars)) {
      if (key !== "reset")
        delete (evalSession.vars as Record<string, unknown>)[key];
    }
    return true;
  };
  const ctx = {
  ...
  try {
        ...
        ? await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () =>
                  reject(
                    new Error(`evaluateCode timed out after ${timeoutMs}ms`),
                  ),
                timeoutMs,
              ),
            ),
    ...
    const maxChars = Math.max(
      100,
      Math.min(200000, Number(maxOutputChars ?? 20000)),
    );
...
async function handleCallRestApi(requestData: unknown) {
  const { method, url, body, query, headers } =
    requestData as CallRestApiRequest;
  if (!method || !url) {
    throw new Error("method and url are required");
  }
  const methodKey = String(method).toLowerCase();
  const client = RestAPI as unknown as Record<
    string,
...
function handleGetStoreState(requestData: unknown) {
  const { storeName, method, args } = requestData as GetStoreStateRequest & {
    method?: string;
    args?: unknown[];
  };
  if (!storeName?.trim()) {
  ...
  const methods = Object.getOwnPropertyNames(proto).filter(
    (n) => n !== "constructor" && typeof (store as any)[n] === "function",
  );
...
function handleGetStoreSubscriptions(requestData: unknown) {
  ...
  for (const entry of entries) {
    ...
    if (moduleId != null) {
      const hit = arr.some(
        (h) => h?.__moduleId === moduleId || h?._moduleId === moduleId,
      );
      if (hit) {
...
function startTrace(
  ...
  const { regex, patternString } = parsePotentialRegex(
    filter ?? "",
    isRegex ?? false,
  );
  const payloadMatcher = createTextMatcher(matchPayload, isRegex);
  traceState.maxEntries = Math.max(
    50,
    Math.min(2000, Number(maxEntries ?? traceState.maxEntries)),
  );
  ...
  if (maxPayloadDepth !== undefined && maxPayloadDepth !== null) {
    traceState.maxPayloadDepth = Math.max(
      1,
      Math.min(10, Number(maxPayloadDepth)),
    );
  }
  const nextSampleRate = Number.isFinite(sampleRate as number)
    ? Number(sampleRate)
    : traceState.sampleRate;
  traceState.sampleRate = Math.max(0, Math.min(1, nextSampleRate || 0));
  traceState.matchPayload = {
    matcher: payloadMatcher,
    isRegex: Boolean(isRegex),
    pattern: matchPayload ?? null,
  };
  ...
  const interceptor = (action: Record<string, unknown>) => {
    ...
    if (
      traceState.sampleRate > 0 &&
      traceState.sampleRate < 1 &&
      Math.random() > traceState.sampleRate
    ) {
      return action;
    }
...
function handleTraceStore(storeName: string) {
  ...
  const removeFn =
    typeof remove === "function" ? () => remove.call(store, handler) : () => {};
...
function handleTrace(requestData: TraceRequest) {
  ...
  if (action === "handlers") {
    return handleDispatcherActions({
      filter: requestData.event ?? filter,
      isRegex,
    });
  }
  ...
  const fieldAllowlist =
    Array.isArray(fields) && fields.length ? fields : traceState.fields;
  ...
  const slice = traceState.events.slice(offset, offset + limit).map((entry) => {
    ...
    return {
      ...entry,
      payload: truncatePayload(depthTrimmed, maxPayloadChars),
    };
  });
  return {
    active: traceState.active,
...
function resolveInterceptTarget(
  ...
  let target: any = exportName
    ? (exports as any)?.[exportName]
    : ((exports as any)?.default ?? exports);
  let parent: any = null;
...
function handleIntercept(requestData: InterceptRequest) {
  ...
  if (action === "status") {
    const items = Array.from(intercepts.values())
      .slice(offset, offset + limit)
      .map((entry) => ({
        id: entry.id,
  ...
  const resolvedId =
    id ?? `${moduleId}:${exportName ?? "default"}:${path ?? "export"}`;
  if (intercepts.has(resolvedId)) {
    return { id: resolvedId, active: true, alreadyActive: true };
  }

  const { exports, target, parent, key, descriptor } = resolveInterceptTarget(
    moduleId,
    exportName,
    path,
  );
...
function handleGetCurrentContext(): GetCurrentContextResult {
  ...
  const selectedChannelId =
    Common.SelectedChannelStore?.getChannelId?.() ?? null;
  const selectedGuildId = Common.SelectedGuildStore?.getGuildId?.() ?? null;
  const channel = selectedChannelId
    ? Common.ChannelStore?.getChannel?.(selectedChannelId)
    : null;
  const guild = selectedGuildId
    ? Common.GuildStore?.getGuild?.(selectedGuildId)
    : null;
  const guildCount =
...
async function handleWaitForReady(requestData?: unknown) {
  const { timeoutMs = 5000, intervalMs = 150 } = (requestData ?? {}) as {
    timeoutMs?: number;
    intervalMs?: number;
  };
...
async function handleFindEnum(requestData: unknown) {
  ...
  } = requestData as FindEnumRequest;
  if (!query?.trim()) {
    throw new Error("query is required");
  }
  const queryText = String(query).trim();
  const key = escapeRegex(queryText);
  const nameQuery = /^[A-Za-z_$][\w$]*$/.test(queryText)
    ? queryText
    : undefined;
  ...
  const freezeRegex = new RegExp(
    String.raw`Object\.freeze\(\s*\{[^}]*\b${key}\b[^}]*\}\s*\)`,
    "g",
  );
  const exportRegex = new RegExp(
    String.raw`exports?\.[A-Za-z_$][\w$]*\s*=\s*\{[^}]*\b${key}\b[^}]*\}`,
    "g",
  );
  const helperRegex = new RegExp(
    String.raw`\b(?:makeEnum|createEnum|enumify)\s*\(\s*\{[^}]*\b${key}\b[^}]*\}\s*\)`,
    "g",
  );
  const entriesRegex = new RegExp(
    String.raw`Object\.fromEntries\(\s*\[[^\]]*\b${key}\b[^\]]*\]\s*\)`,
    "g",
  );
  const results: Array<{
    moduleId: number;
  ...
  if (nameQuery) {
    const nameDeclRegex = new RegExp(
      String.raw`(?:const|let|var)\s+${nameKey}\s*=\s*\{([^}]*)\}`,
      "g",
    );
    const nameExportRegex = new RegExp(
      String.raw`exports?\.\s*${nameKey}\s*=\s*\{([^}]*)\}`,
      "g",
    );
    for (const id in wreq.m) {
      if (results.length >= limit) break;
  ...
  for (const id in wreq.m) {
    ...
    if (match) {
      ...
      if (includeMembers) {
          ...
          if (exported && typeof exported === "object") {
            ...
            for (const value of Object.values(
              ...
              if (
                value &&
                typeof value === "object" &&
                queryText in (value as Record<string, unknown>)
              ) {
                resolvedMembers = Object.keys(
        ...
        (results[results.length - 1] as any).memberCount =
...
function handleSearchIntlInModule(requestData: unknown) {
  ...
  const seen = new Map<
    string,
    { key: string; modifier?: string; hashed: string }
  >();
...
function handlePluginSettings(requestData: unknown) {
  const { pluginId, pluginName, name, action, values } =
    requestData as PluginSettingsRequest & {
      pluginName?: string;
      name?: string;
    };
  ...
  const settings =
    Settings.plugins[resolved] ??
    (Settings.plugins[resolved] = { enabled: false });
  ...
  const validate = (key: string, value: unknown) => {
    ...
    if (!opt)
      return { valid: false, error: "Unknown setting", typeName: "unknown" };
  ...
  const entries = Object.keys(options).map((key) => {
    ...
    const defaultValue =
      opt?.default ??
      (opt?.type === 4
        ? opt?.options?.find((o: any) => o.default)?.value
        : undefined);
    return {
      key,
      value: settings[key],
...
function handleQueryDom(requestData: unknown) {
  ...
  try {
    ...
    const nodes = all
      ...
      .map((el) => summarizeNode(el, includeText, includeAttrs, maxTextLength));
    return {
      nodes,
...
function handleGetElementStyles(requestData: unknown) {
  const { selector, properties } = requestData as {
    selector: string;
    properties?: string[];
  };
  const el = document.querySelector(selector);
  if (!el)
    return { found: false, selector, error: "No element matches selector" };
...
function handleModifyElement(requestData: unknown) {
  const { selector, styles, addClass, removeClass, setAttribute } =
    requestData as {
      selector: string;
...
function handleGetComponentTree(requestData: unknown) {
  const { selector, maxDepth = 50 } = requestData as {
    selector: string;
    maxDepth?: number;
  };
  const el = document.querySelector(selector);
  ...
  while (fiber && depth < Math.min(maxDepth, 100)) {
    if (fiber.type) {
      ...
      if (name && name.length > 1) {
        ...
        if (hasProps) {
          entry.propKeys = Object.keys(fiber.memoizedProps as object).slice(
            0,
            20,
          );
        }
...
function getComponentInfoFromFiber(fiber: any) {
  ...
  const resolved = moduleId
    ? resolveComponentNameFromModule(moduleId, name)
    : null;
  return {
    name,
    moduleId,
...
function handleGetComponentTreeDetailed(requestData: unknown) {
  ...
  const tree = buildFiberTree(
    fiber,
    Math.min(maxDepth, 50),
    Math.min(maxBreadth, 25),
  );
...
function handleListDomClasses(requestData: unknown) {
  const { maxNodes = 2000, maxClasses = 50 } =
    requestData as ListDomClassesRequest;
  ...
  while (node) {
    ...
    if (text.length > 0) {
      const matchesSearch = matcher
        ? ((matcher.lastIndex = 0), matcher.test(text))
        : text.includes(query);
      if (matchesSearch) {
        totalFound++;
...
export function initWs(isManual = false) {
  ...
  dynamicReconnectAttempts = Math.max(
    1,
    Math.min(20, settings.store.maxReconnectAttempts || 5),
  );
  dynamicFallbackSpread = Math.max(
    0,
    Math.min(MAX_SCAN_SPREAD, settings.store.scanSpread || 2),
  );
  if (settings.store.prebuildSearchIndex) {
    void ModuleSearchEngine.ensureTokenIndex();
  }
  ...
  if (!hasUserInput) {
    fallbackTimer = setTimeout(() => {
      const anyOpen = Array.from(sockets.values()).some(
        (sock) => sock.readyState === WebSocket.OPEN,
      );
      if (anyOpen) return;
      const fallbackPorts = generateFallbackPorts().filter(
        (p) => !attempted.has(p),
      );
  ...
  try {
    ...
    ws.addEventListener("close", () => {
      ...
      const delay = Math.min(
        INITIAL_RECONNECT_DELAY * (1 << state.reconnectAttempts),
        MAX_RECONNECT_DELAY,
      );
      state.reconnectAttempts++;
      if (state.reconnectAttempts < dynamicReconnectAttempts) {
        state.reconnectTimeout = setTimeout(
          () => ensureConnection(port, false),
          delay,
        );
      } else {
        recordStoppedPort(port);
    ...
    ws.addEventListener("message", async (e) => {
      ...
      const limiter =
        requestLimiterPerPort.get(port) || new Map<string, number>();
      ...
      if (now - lastRequest < REQUEST_LIMIT_TIME) {
        ...
        if (recentCount > MAX_REQUESTS_PER_TYPE) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, REQUEST_LIMIT_TIME),
          );
        }
      }
