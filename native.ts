...
export async function startServer(
  ...
  for (const [id, resolve] of pendingRequests) {
    resolve({
      jsonrpc: "2.0",
      id,
      error: { code: -32603, message: "Session reset" },
    });
  }
  ...
  return await new Promise<StartServerResult>((resolve) => {
    ...
    nextServer.listen(port, HOST, () => {
      ...
      console.log(
        `DevCompanionExtended: IPC MCP server listening on ${HOST}:${actualPort}`,
      );
...
export function stopServer(_: IpcMainInvokeEvent): OkResult {
  for (const [id, resolve] of pendingRequests) {
    resolve({
      jsonrpc: "2.0",
      id,
      error: { code: -32603, message: "Server stopped" },
    });
  }
  pendingRequests.clear();
  requestQueue.length = 0;
...
export function getNextRequest(
  _: IpcMainInvokeEvent,
): { id: number; request: MCPRequest } | null {
