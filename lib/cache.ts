...
>();
export const toolResponseCache = new Map<
  string,
  { timestamp: number; ttlMs: number; value: unknown }
>();
...
export const patchedModulesCache = new Map<
  string,
  {
    timestamp: number;
    result: {
...
>();
export const patchedModulesInFlight = new Map<
  string,
  Promise<{
    patches: Array<{
      moduleId: number;
...
export const componentNameCache = new Map<
  number,
  { name: string; source: string; confidence: number } | null
>();

export function isCacheableTool(name: string, args?: unknown): boolean {
  if (
    ["reloadDiscord", "evaluateCode", "batch_tools", "analytics"].includes(name)
  ) {
    return false;
  }
...
export function getToolCacheTtlMs(name: string): number {
  ...
  return (
    TOOL_CACHE_OVERRIDES.get(name) ??
    settings.store.cacheTtlMs ??
    TOOL_CACHE_TTL_MS
  );
}
...
export function cleanupToolCache(): void {
  ...
  const entries = Array.from(toolResponseCache.entries()).sort(
...
export async function executeWithCache<T>(
  ...
  if (!settings.store.cacheEnabled || !isCacheableTool(name, args))
    return compute();
  ...
  const task = (async () => {
    ...
    if (shouldCacheResult(value)) {
      const cachedValue =
        value &&
        typeof value === "object" &&
        "cached" in (value as Record<string, unknown>)
      ...
      toolResponseCache.set(cacheKey, {
        timestamp: Date.now(),
        ttlMs: adaptiveTtlMs,
        value: cachedValue,
      });
...
export class ModuleSearchEngine {
  ...
  private static searchIndex = new Map<
    string,
    { ids: Set<string>; timestamp: number }
  >();
  private static literalCache = new Map<
    string,
    {
      timestamp: number;
      matches: Array<{
  ...
  ): AsyncGenerator<{ id: string; code: string }, void, unknown> {
  ...
  static async ensureTokenIndex(): Promise<void> {
    ...
    this.tokenIndexInFlight = (async () => {
        ...
        for (const id of batch) {
          ...
          for (const token of tokens) {
            if (!this.addToken(token, id)) {
              ...
              this.tokenIndex.clear();
              logger.warn(
                "Prebuilt search index disabled: token limit exceeded",
              );
              return;
            }
          }
      ...
      debugInfo(
        `Search index ready: ${processed} modules, ${this.tokenIndex.size} tokens`,
      );
    })().finally(() => {
      this.tokenIndexInFlight = null;
    });
  ...
  static async getIndexedCandidates(
    query: string,
  ): Promise<Set<string> | null> {
    if (!settings.store.prebuildSearchIndex || this.tokenIndexDisabled)
      return null;
