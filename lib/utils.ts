...
export function parseRegexArg(arg: string | { pattern: string; flags: string; }): string | RegExp {
...
export function parseRegexString(arg: string): { type: "string"; value: string; } | { type: "regex"; value: { pattern: string; flags: string; }; } {
    if (arg.startsWith("/")) {
        const lastSlash = arg.lastIndexOf("/");
        if (lastSlash > 0) {
            const pattern = arg.substring(1, lastSlash);
...
export function parsePotentialRegex(query: string, isRegex: boolean): { regex: RegExp | null; patternString: string | null; } {
...
export function truncatePayloadDepth(payload: unknown, maxDepth?: number | null, depth = 0): unknown {
...
export const SENSITIVE_KEY_REGEX = /token|authorization|cookie|password|pass|email|phone|secret/i;
export const TOKEN_VALUE_REGEX = /[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{10,}/g;

export function filterFields(value: unknown, fields?: string[] | null): unknown {
...
export function ensureBaseFields(entry: Record<string, unknown>, base: Record<string, unknown>) {
...
export function getLineContext(text: string, index: number, contextLines: number) {
...
export function getMatchContext(text: string, pattern: string | RegExp, contextLines = 2, radius = 120): MatchContextResult {
...
export function sliceSnippets(text: string, matches: RegExpMatchArray[] | null, limit: number, radius = 60): string[] {
    if (!matches) return [];
...
export function extractStringLiterals(source: string, minLength = 4, maxLength = 80, limit = 20) {
...
export function collectAttributes(el: Element, max = 10): Record<string, string> {
...
export function summarizeNode(el: Element, includeText: boolean, includeAttrs: boolean, maxTextLength: number): DomNodeSummary {
...
export function buildTree(el: Element, depth: number, breadth: number, includeText: boolean, maxTextLength: number): DomNodeSummary {
